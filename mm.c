...
int mm_init(void)
{
    ...
    // 建立空heap
    if((heap_listp = mem_sbrk(12 * WSIZE + CHUNKSIZE)) == (void *)-1) 
        return -1;
    // padding
    *(size_t *)heap_listp = 0; 
    
    *(size_t *)(heap_listp + WSIZE) = NULL; // 0-32
    *(size_t *)(heap_listp + WSIZE*2) = NULL; // 32-64
    *(size_t *)(heap_listp + WSIZE*3) = NULL; // 64-128
    *(size_t *)(heap_listp + WSIZE*4) = NULL; // 128-256
    *(size_t *)(heap_listp + WSIZE*5) = NULL; // 256-512
    *(size_t *)(heap_listp + WSIZE*6) = NULL; // 512-1024
    *(size_t *)(heap_listp + WSIZE*7) = NULL; // 1024-2048
    *(size_t *)(heap_listp + WSIZE*8) = NULL; // 2048-4096
    *(size_t *)(heap_listp + WSIZE*9) = heap_listp + WSIZE*11; // 4096-
    // padding
    *(size_t *)(heap_listp + WSIZE*10) = WSIZE | 1;

    // 结尾块
    *(size_t *)(heap_listp + WSIZE*11 + CHUNKSIZE) = 1;
    // 空闲块header
    *(size_t *)(heap_listp + WSIZE*11) = CHUNKSIZE;

    // 空闲块link_prev_p 
    *(size_t *)(heap_listp + WSIZE*12) = NULL;
    // 空闲块link_next_p
    *(size_t *)(heap_listp + WSIZE*13) = NULL;
    // 空闲块footer
    *(size_t *)(heap_listp + WSIZE*11 + CHUNKSIZE - WSIZE) = CHUNKSIZE;

    return 0;
}

void show_free_list(int list_num){
    // <32
    char *link_list = *(size_t *)((char *)mem_heap_lo() + list_num * WSIZE);
    int count = 0;
...
int find_fit_list(size_t newsize){
    int list_num = 0;
    if (newsize < 32){
        list_num = 1;
    }
...
void *find_fit(size_t newsize){
    ...
    int list_num = find_fit_list(newsize);

    while (list_num <= 9 && free_hp == NULL)
    {
        char *heap_listp = *(size_t *)(mem_heap_lo() + list_num * WSIZE);
        if(heap_listp == NULL){
            list_num ++;
            continue;
        }
...
void *coalesce(void *ptr)
{
    ...
    if((size_t)next_hp > (size_t)mem_heap_hi())
        next_hp = NULL;
    ...
    if(( (*(size_t *)prev_hp) & 1 ) == 0){
        ...
        {
            int list_num = find_fit_list(*(size_t *)hp);
            change_free_list_head(link_next_hp, list_num);
        }
    ...
    //判断下一个是否为空闲块
    if(next_hp != NULL &&  ( (*(size_t *)next_hp) & 1 ) == 0 ){
        ...
        {
            int list_num = find_fit_list(*(size_t *)next_hp);
            change_free_list_head(link_next_hp, list_num);
        }
    ...
    *(size_t *)(hp + total_size - WSIZE) = total_size;
    ...
    int list_num = find_fit_list(total_size);
    // 将hp加入link_list的头部
    char *link_list_head = *(size_t *)((char *)mem_heap_lo() + WSIZE*list_num);
    ...
    *(size_t *)(hp + WSIZE*2) = link_list_head;
    ...
    change_free_list_head(hp, list_num);
...
void change_free_list_head(char * new_head, int list_num){
    *(size_t *)((char *)mem_heap_lo() + WSIZE*list_num) = new_head;
}
...
void *extend_heap(){
    ...
    *(size_t *)new_end_p = 1;
    ...
    char *free9_list_head = *(size_t *)((char *)mem_heap_lo() + 9*WSIZE);
    char *hp = end_p;

    if (free9_list_head != NULL)
    {
    ...
    */
    ...
    if(newsize <= free_size-4*WSIZE){
        ...
        *(size_t *)(hp + free_size - WSIZE) = free_size - newsize;
        ...
        int list_num = find_fit_list(free_size - newsize);
        // 将hp加入link_list的头部
        char *link_list_head = *(size_t *)((char *)mem_heap_lo() + WSIZE*list_num);

        // 更改hp的两个指针
        *(size_t *)(rest_free_block + WSIZE) = NULL;
        *(size_t *)(rest_free_block + WSIZE*2) = link_list_head;

        // 更改link_list的link_prev_hp指针
        if(link_list_head != NULL){
            *(size_t *)(link_list_head + WSIZE) = rest_free_block;
        }
        
        change_free_list_head(rest_free_block, list_num);
        *(size_t *)hp = newsize | 1;
        // 更改分配块footer
...
*/
    ...
    void *old_head = ptr - WSIZE;
    size_t old_size = (*(size_t *)old_head) & -2;
    void *next_block = old_head + old_size;

    if((size_t)next_block > (size_t)mem_heap_hi()){
        next_block = NULL;
    }
    ...
    if(old_size >= newsize){
        ...
        // 如果后边的块为空闲块
        if(next_block != NULL && (*(size_t *)next_block & 1) == 0){

            size_t free_size = *(size_t *)next_block;
            size_t total_size = free_size + old_size;

            // 如果空闲块足够大，而且还能剩余一个空闲块
            if(total_size >= (newsize + 4*WSIZE)){

                // 将prev_hp的 link_prev_hp 和 link_next_hp 连上
                char *link_prev_hp = *(size_t *)(next_block + WSIZE);
                char *link_next_hp = *(size_t *)(next_block + 2*WSIZE);
                ...
                {
                    int list_num = find_fit_list(*(size_t *)next_block);
                    change_free_list_head(link_next_hp, list_num);
                }
                
                //更改剩余空闲块header
                char *rest_free_block = (char *)old_head + newsize;
                *(size_t *)(rest_free_block) = total_size - newsize;
                // 更改剩余空闲块footer
                *(size_t *)(old_head + total_size - WSIZE) = total_size - newsize;

                int list_num = find_fit_list(total_size - newsize);
                // 将hp加入link_list的头部
                char *link_list_head = *(size_t *)((char *)mem_heap_lo() + WSIZE*list_num);
                ...
                *(size_t *)old_head = newsize | 1;
                // 更改分配块footer
                *(size_t *)(old_head + newsize - WSIZE) = newsize | 1;
                newptr = ptr;
                return newptr;
            }
            else if(free_size + old_size >= newsize){
                // 整个free_block作为新的block, 多余的作为padding
                // 将hp的 link_prev_hp 和 link_next_hp 连上
                char *link_prev_hp = *(size_t *)(next_block + WSIZE);
                char *link_next_hp = *(size_t *)(next_block + 2*WSIZE);
                ...
                // 更改分配快footer
                *(size_t *)old_head = total_size | 1;
                *(size_t *)(old_head + total_size - WSIZE) = total_size | 1;
                newptr = ptr;
                return newptr;
            }
        }
